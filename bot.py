...
import shutil
import asyncio
import asyncio as _asyncio_jio
import gzip
import base64
import hashlib
...
async def _group_gate(client: Client, message: Message):
    ...
    _DM_ALLOWED_PREFIXES = (
        ...
        "/login", "/otp",                           # JioTV login (owner)
        "/live", "/rec", "/jiostatus",              # JioTV commands
    )
...
_BOT_COMMANDS = [
    ...
    # JioTV commands
    BotCommand("login",          "JioTV login — OTP bhejo (owner only)"),
    BotCommand("otp",            "JioTV OTP verify karo (owner only)"),
    BotCommand("live",           "JioTV live stream link lo"),
    BotCommand("channels",       "JioTV channel list dekho"),
    BotCommand("jiostatus",      "JioTV login status check karo"),
]
...
# ---------------------------------------------------------------------------
# JioTV — Real API Integration (TS-JioTV-Bot)
# Login: /login <mobile> → /otp <code>
# Live:  /live <channel>
# Rec:   /rec <channel> -t HH:MMAM - HH:MMPM
# ---------------------------------------------------------------------------

import requests as _requests
from zoneinfo import ZoneInfo as _ZoneInfo

_JIO_DATA_FOLDER = os.path.join("app", "assets", "data")
_JIO_CHANNEL_URL = base64.b64decode(
    "aHR0cHM6Ly9taXR0aHU3ODYuZ2l0aHViLmlvL3R2ZXBnL2ppb3R2L2ppb2RhdGEuanNvbg=="
).decode()
_IST = _ZoneInfo("Asia/Kolkata")
_jio_channel_cache: list | None = None


# ── Credential helpers ──────────────────────────────────────────────────────

def _jio_encrypt(data: str, key: str) -> str:
    k = int(key)
    enc = "".join(chr(ord(c) + k) for c in data)
    return base64.b64encode(enc.encode("latin-1")).decode()


def _jio_save_credentials(jio_data: dict, mobile: str) -> None:
    u_name = _jio_encrypt(mobile, "TS-JIOTV")
    os.makedirs(_JIO_DATA_FOLDER, exist_ok=True)
    creds_path = os.path.join(_JIO_DATA_FOLDER, "creds.jtv")
    key_path   = os.path.join(_JIO_DATA_FOLDER, "credskey.jtv")
    with open(creds_path, "w") as f:
        f.write(_jio_encrypt(json.dumps(jio_data), u_name))
    with open(key_path, "w") as f:
        f.write(u_name)


def _jio_load_credentials() -> dict | None:
    creds_path = os.path.join(_JIO_DATA_FOLDER, "creds.jtv")
    key_path   = os.path.join(_JIO_DATA_FOLDER, "credskey.jtv")
...
def _jio_is_logged_in() -> bool:
    return _jio_load_credentials() is not None


# ── JioTV API helpers ───────────────────────────────────────────────────────

def _jio_get_channels() -> list:
    global _jio_channel_cache
    if _jio_channel_cache:
        return _jio_channel_cache
    resp = _requests.get(_JIO_CHANNEL_URL, timeout=10)
    resp.raise_for_status()
    _jio_channel_cache = resp.json()
    return _jio_channel_cache


def _jio_find_channel(name_query: str) -> dict | None:
    channels = _jio_get_channels()
...
def _jio_get_epg(channel_id, offset=0) -> dict | None:
    url = (
...
def _jio_parse_time(time_str: str):
...
def _jio_find_program(channel_id, start_h, start_m, end_h, end_m):
    for offset in [0, -1, 1]:
        epg = _jio_get_epg(channel_id, offset)
        if not epg:
            continue
        for program in epg.get("epg", []):
            try:
                p_start = datetime.fromtimestamp(int(program.get("startEpoch", 0)), tz=_IST)
                p_end   = datetime.fromtimestamp(int(program.get("endEpoch",   0)), tz=_IST)
...
def _jio_send_otp(mobile: str) -> dict:
    ...
    try:
        ...
        if resp.status_code == 204:
            return {"status": "success", "message": "OTP bhej diya!"}
        data = resp.json()
        return {"status": "error", "message": data.get("message", f"Error {resp.status_code}")}
    except Exception as e:
        return {"status": "error", "message": str(e)}


def _jio_verify_otp(mobile: str, otp: str) -> dict:
    ...
    if data.get("ssoToken"):
        _jio_save_credentials(data, mobile)
...
def _jio_get_stream_url(channel_id, creds: dict) -> str | None:
    ...
    headers = {
        ...
        "userid": crm, "crmid": crm, "deviceId": device_id,
        "devicetype": "phone", "isott": "true", "languageId": "6",
        "lbcookie": "1", "os": "android", "dm": "Xiaomi 22101316UP",
        "osversion": "14", "srno": "250918144000",
        "accesstoken": access_token, "subscriberid": crm,
        "uniqueId": unique_id, "usergroup": "tvYR7NSNn7rymo3F",
        "User-Agent": "okhttp/4.12.13", "versionCode": "452",
    }
    try:
        resp = _requests.post(
...
def _jio_get_catchup_url(channel_id, srno, begin, end, creds: dict) -> str | None:
    access_token = creds.get("authToken", "")
    crm          = creds.get("sessionAttributes", {}).get("user", {}).get("subscriberId", "")
    unique_id    = creds.get("sessionAttributes", {}).get("user", {}).get("unique", "")
    device_id    = creds.get("deviceId", "")
    post_data    = (
        f"stream_type=Catchup&channel_id={channel_id}&programId={srno}"
        f"&showtime=000000&srno={srno}&begin={begin}&end={end}"
    )
    headers = {
        "Host": "jiotvapi.media.jio.com",
        "Content-Type": "application/x-www-form-urlencoded",
...
# In-memory state for /login → /otp flow
_jio_pending_mobile: dict[int, str] = {}


# ── Pyrogram command handlers ───────────────────────────────────────────────

# /login <mobile>  — OWNER ONLY
@app.on_message(filters.command(["login"]) & _OWNER_FILTER)
async def jio_login_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text(
            "**Usage:** `/login <10-digit mobile>`\n"
    ...
    if not re.match(r"^\d{10}$", mobile):
        return await message.reply_text(
            "❌ Sirf 10-digit number — country code mat dalna.\nExample: `/login 9876543210`",
            parse_mode=enums.ParseMode.MARKDOWN
        )

    status = await message.reply_text(f"📲 OTP bhej raha hoon **{mobile}** pe...")
    result = await asyncio.get_event_loop().run_in_executor(None, _jio_send_otp, mobile)
    if result["status"] == "success":
        _jio_pending_mobile[message.from_user.id] = mobile
        await status.edit_text(
            f"✅ OTP **{mobile}** pe bhej diya!\n"
            "Ab `/otp <6-digit code>` se verify karo.",
            parse_mode=enums.ParseMode.MARKDOWN
        )
    else:
        await status.edit_text(f"❌ OTP fail: {result['message']}")


# /otp <code>  — OWNER ONLY
@app.on_message(filters.command(["otp"]) & _OWNER_FILTER)
async def jio_otp_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text(
            "**Usage:** `/otp <6-digit code>`",
            parse_mode=enums.ParseMode.MARKDOWN
        )
    otp = args[1].strip()
    if not re.match(r"^\d{6}$", otp):
        return await message.reply_text("❌ 6-digit OTP daliye. Example: `/otp 123456`", parse_mode=enums.ParseMode.MARKDOWN)

    uid    = message.from_user.id
    mobile = _jio_pending_mobile.get(uid)
    if not mobile:
        return await message.reply_text("❌ Pehle `/login <mobile>` se OTP request karo.", parse_mode=enums.ParseMode.MARKDOWN)

    status = await message.reply_text("🔐 OTP verify ho raha hai...")
    result = await asyncio.get_event_loop().run_in_executor(None, _jio_verify_otp, mobile, otp)
    if result["status"] == "success":
        _jio_pending_mobile.pop(uid, None)
        await status.edit_text(
            "✅ **JioTV Login Successful!**\n\n"
            "Commands:\n"
            "• `/live <channel>` — Live stream link\n"
            "• `/rec <channel> -t HH:MMAM - HH:MMPM` — Catchup recording\n"
            "• `/channels` — Channel list",
            parse_mode=enums.ParseMode.MARKDOWN
        )
    else:
        await status.edit_text(
            f"❌ Verify fail: {result['message']}\nDobara try karo: `/otp <code>`",
            parse_mode=enums.ParseMode.MARKDOWN
        )


# /jiostatus  — AUTH
@app.on_message(filters.command(["jiostatus"]) & AUTH)
async def jio_status_cmd(client: Client, message: Message):
    creds = _jio_load_credentials()
    if not creds:
        return await message.reply_text(
            "❌ JioTV login nahi hai.\nOwner se kaho `/login <mobile>` kare.",
            parse_mode=enums.ParseMode.MARKDOWN
        )
    try:
        mobile = creds.get("sessionAttributes", {}).get("user", {}).get("mobile", "N/A")
        ...
        if jwt:
            ...
            if len(parts) > 1:
                ...
                expiry  = datetime.fromtimestamp(exp, tz=_IST).strftime("%d-%b-%Y %I:%M %p IST")
    except Exception:
        mobile, name, expiry = "N/A", "", "N/A"

    await message.reply_text(
        f"✅ **JioTV Login Active**\n\n"
        f"👤 Name: `{name}`\n"
        f"📱 Mobile: `{mobile}`\n"
        f"⏳ Token Expiry: `{expiry}`\n\n"
        "Commands:\n"
        "• `/live <channel>` — Live stream\n"
        "• `/rec <channel> -t HH:MMAM - HH:MMPM` — Catchup\n"
        "• `/channels` — Channel list",
        parse_mode=enums.ParseMode.MARKDOWN
    )


# /live <channel name>  — AUTH
@app.on_message(filters.command(["live"]) & AUTH)
async def jio_live_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text("❌ Usage: `/live <channel name>`\nExample: `/live Star Sports 1`", parse_mode=enums.ParseMode.MARKDOWN)

    creds = _jio_load_credentials()
    if not creds:
        return await message.reply_text("❌ JioTV login nahi hai.\nOwner se kaho `/login <mobile>` kare.", parse_mode=enums.ParseMode.MARKDOWN)

    channel_name = " ".join(args[1:])
    status = await message.reply_text(f"🔍 **{channel_name}** dhundh raha hoon...", parse_mode=enums.ParseMode.MARKDOWN)

    channel = await asyncio.get_event_loop().run_in_executor(None, _jio_find_channel, channel_name)
    if not channel:
        return await status.edit_text(
            f"❌ Channel **{channel_name}** nahi mila.\n`/search {channel_name}` try karo.",
            parse_mode=enums.ParseMode.MARKDOWN
        )

    stream_url = await asyncio.get_event_loop().run_in_executor(
        None, _jio_get_stream_url, channel["channel_id"], creds
    )
    if not stream_url:
        return await status.edit_text("❌ Stream URL nahi mili. Token expire ho sakta hai — `/login` dobara karo.")
...
# /rec <channel> -t HH:MMAM - HH:MMPM  — AUTH
@app.on_message(filters.command(["rec"]) & AUTH)
async def jio_rec_cmd(client: Client, message: Message):
    args = message.command
    if len(args) < 2:
        return await message.reply_text(
    ...
    if not match:
        return await message.reply_text(
            "❌ **Format sahi nahi.**\n\n"
            "Sahi: `/rec <channel> -t HH:MMAM - HH:MMPM`\n"
    ...
    start_h, start_m = _jio_parse_time(start_time_str)
    end_h,   end_m   = _jio_parse_time(end_time_str)

    if start_h is None or end_h is None:
        return await message.reply_text("❌ Time format sahi nahi. Example: `12:00PM`", parse_mode=enums.ParseMode.MARKDOWN)
    ...
    if channel.get("isCatchupAvailable") != "True":
        return await status.edit_text(
            f"❌ **{channel['channel_name']}** par catchup available nahi hai.",
            parse_mode=enums.ParseMode.MARKDOWN
        )

    channel_id = channel["channel_id"]
    program, p_start, p_end = await asyncio.get_event_loop().run_in_executor(
        None, _jio_find_program, channel_id, start_h, start_m, end_h, end_m
    )

    if not program:
        return await status.edit_text(
            f"❌ **{channel['channel_name']}** par `{start_time_str}` ka koi program nahi mila.\n\n"
            "EPG mein show nahi hai ya time galat hai.",
            parse_mode=enums.ParseMode.MARKDOWN
        )
    ...
    stream_url = await asyncio.get_event_loop().run_in_executor(
        None, _jio_get_catchup_url, channel_id, srno, begin, end_epoch, creds
    )
    if not stream_url:
        return await status.edit_text(
            "❌ Catchup stream URL nahi mili.\nToken ya program issue ho sakta hai."
        )
    ...
    await status.edit_text(text, parse_mode=enums.ParseMode.MARKDOWN)


# /channels  — AUTH
@app.on_message(filters.command(["channels"]) & AUTH)
async def jio_channels_cmd(client: Client, message: Message):
    status = await message.reply_text("📋 Channels list load ho rahi hai...")
    try:
        channels = await asyncio.get_event_loop().run_in_executor(None, _jio_get_channels)
    except Exception as e:
        return await status.edit_text(f"❌ Channels load nahi ho sake: `{e}`")
    ...
    for cat in sorted(categories.keys()):
        ...
        rows.append([InlineKeyboardButton(f"{cat} ({count})", callback_data=f"jioc_cat:{cat}")])
...
@app.on_callback_query(filters.regex(r"^jioc_cat:"))
async def jio_cat_cb(client: Client, cq: CallbackQuery):
    cat = cq.data.split(":", 1)[1]
    try:
        channels   = await asyncio.get_event_loop().run_in_executor(None, _jio_get_channels)
        cat_chs    = [c for c in channels if c.get("channelCategoryId") == cat]
        lines      = [f"📺 **{cat}** ({len(cat_chs)} channels)\n"]
        for ch in cat_chs:
        ...
        if len(text) > 4000:
            text = text[:4000] + "\n..."
        back_kb = InlineKeyboardMarkup([[InlineKeyboardButton("« Back", callback_data="jioc_back")]])
        await cq.message.edit_text(text, reply_markup=back_kb, parse_mode=enums.ParseMode.MARKDOWN)
    except Exception as e:
        await cq.answer(f"Error: {e}", show_alert=True)
    await cq.answer()


@app.on_callback_query(filters.regex(r"^jioc_back$"))
async def jio_back_cb(client: Client, cq: CallbackQuery):
    try:
        channels   = await asyncio.get_event_loop().run_in_executor(None, _jio_get_channels)
        categories: dict = {}
        for ch in channels:
            ...
            categories.setdefault(cat, []).append(ch["channel_name"])
        rows = [
            [InlineKeyboardButton(f"{cat} ({len(v)})", callback_data=f"jioc_cat:{cat}")]
